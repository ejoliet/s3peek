...
- **2026-05-21 (decided)**: `peek --deep` on `s3://` URIs fails fast with exit 1 when `meta.size > cfg.max_range_get_bytes`. Deep-inspect requires a complete byte stream; truncated Range-GET silently corrupts ASDF/FITS parsing. Future fix tracked in `.agents/plans/future-deep-s3-max-bytes-config.md`.
